...
(function ($, window) {
    ...
    FreezeTable.prototype.init = function () {
        ...
        try {
            ...
            if (!this.$table.length) {
                console.error("The element must contain a table dom 02");
            }
        }
        ...
        if (!this.scrollable) {
            this.$tableWrapper.css('min-height', '100%');
    ...
    FreezeTable.prototype.buildHeadTable = function () {
        ...
        let $headTable = this.clone(this.$table);

        // Fast Mode
        if (this.fastMode) {
            $headTable = this.simplifyHead($headTable);
        }
    ...
    FreezeTable.prototype.buildColumnTable = function () {
        ...
        let firsRowColumnHeight = $('.eventFirst > tbody > tr:nth-child(1) ').css('height');
        console.log('firsRowColumnHeight: ' + firsRowColumnHeight);
        this.$columnTableWrap = $('<div class="' + this.columnWrapClass + '"></div>')
            ...
            .css('visibility', 'hidden')
            .css('z-index', 1)
            .css('padding-bottom', '17px');
        this.$columnTableWrap.find('.eventFirst > tbody > tr:nth-child(1) > td').css('height', firsRowColumnHeight);
        // Shadow option
        if (this.shadow) {
    ...
    FreezeTable.prototype.isInit = function () {
        ...
        if (this.$tableWrapper.find("." + this.columnHeadWrapClass).length)
            return true;
        return !!this.$tableWrapper.find("." + this.scrollBarWrapClass).length;
